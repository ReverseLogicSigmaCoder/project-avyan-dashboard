...
def run_real_dast_probe(target_url):
    results = {"missing_headers": [], "vulnerabilities": [], "http_status": "N/A", "server_banner": "Unknown", "endpoint_checks": []}
    try:
        ...
        # 2. Sensitive Endpoint Discovery Check
        endpoints_to_test = ["/admin", "/.env", "/api/v1", "/swagger.json"]
        base_domain = target_url.rstrip('/')
        for ep in endpoints_to_test:
            try:
                test_req = urllib.request.Request(base_domain + ep, headers={'User-Agent': 'SUDARSHAN-Scanner/1.0'})
                with urllib.request.urlopen(test_req, context=ctx, timeout=3) as ep_res:
